...
void Ana(const char *inputFile)
{
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    {    
     ...
     if(abs(gen->PID)==13&& abs(genm->PID)==24) cout<<"pid "<<gen->PID<<" pidm "<<genm->PID<<endl;
     
     //if(abs(gen->PID)==13)cout<<"N: "<<i<<", St: "<<gen->Status<<", PID: "<<gen->PID<<", E: "<<gen->E<<", Px: "<<gen->Px<<", Py: "<<gen->Py<<", Pz: "<<gen->Pz<<", M: "<<gen->Mass<<", M1: "<<gen->M1<<", M2: "<<gen->M2<<", D1: "<<gen->D1<<", D2: "<<gen->D2<<endl;
    }
  }
}
