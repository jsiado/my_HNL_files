...
C     
C     Process: s c~ > mu- nr WEIGHTED<=4 @1
C     *   Decay: nr > mu- mu+ vm WEIGHTED<=4
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     Needed for v4 models
      ...
      DOUBLE PRECISION FK_MDL_WC
      DOUBLE PRECISION FK_ZERO
      DOUBLE PRECISION FK_MDL_WNR
...
C     ----------
      IF (FIRST) THEN
        FIRST=.FALSE.
        FK_ZERO = SIGN(MAX(ABS(ZERO), ABS(MDL_MMU*SMALL_WIDTH_TREATMENT)
     $   ), ZERO)
        FK_MDL_WW = SIGN(MAX(ABS(MDL_WW), ABS(MDL_MW
     $   *SMALL_WIDTH_TREATMENT)), MDL_WW)
        FK_ZERO = SIGN(MAX(ABS(ZERO), ABS(MDL_MS*SMALL_WIDTH_TREATMENT)
     $   ), ZERO)
        FK_MDL_WZ = SIGN(MAX(ABS(MDL_WZ), ABS(MDL_MZ
     $   *SMALL_WIDTH_TREATMENT)), MDL_WZ)
        FK_MDL_WC = SIGN(MAX(ABS(MDL_WC), ABS(MDL_MC
     $   *SMALL_WIDTH_TREATMENT)), MDL_WC)
     ...
     $   , ZERO)
        FK_MDL_WNR = SIGN(MAX(ABS(MDL_WNR), ABS(MDL_MNR
     $   *SMALL_WIDTH_TREATMENT)), MDL_WNR)
      ENDIF
      ...
      CALL FFS6_3(W(1,5),W(1,4),GC_49,MDL_MH, FK_MDL_WH,W(1,7))
      CALL FFS5C1_2(W(1,6),W(1,7),GC_40,MDL_MNR, FK_MDL_WNR,W(1,8))
      CALL FFV6_3(W(1,1),W(1,2),GC_18,MDL_MW, FK_MDL_WW,W(1,7))
C     Amplitude(s) for diagram number 1
      CALL FFV6_0(W(1,8),W(1,3),W(1,7),GC_35,AMP(1))
      CALL FFV6_8_3(W(1,5),W(1,4),GC_23,GC_27,MDL_MZ, FK_MDL_WZ,W(1,8))
      CALL FFV6C1_2(W(1,6),W(1,8),GC_36,MDL_MNR, FK_MDL_WNR,W(1,4))
C     Amplitude(s) for diagram number 2
      CALL FFV6_0(W(1,4),W(1,3),W(1,7),GC_35,AMP(2))
      CALL OXXXXX(P(0,6),ZERO,NHEL(6),+1*IC(6),W(1,4))
      CALL FFV6_3(W(1,5),W(1,4),GC_13,MDL_MW, FK_MDL_WW,W(1,8))
      CALL IXXXXX(P(0,4),MDL_MMU,NHEL(4),-1*IC(4),W(1,4))
      CALL FFV6C1_2(W(1,4),W(1,8),GC_35,MDL_MNR, FK_MDL_WNR,W(1,5))
C     Amplitude(s) for diagram number 3
      CALL FFV6_0(W(1,5),W(1,3),W(1,7),GC_35,AMP(3))
